...
namespace CreateWorkbooks
{
    public partial class ThisAddIn
    {
        ...
        private Microsoft.Office.Interop.Excel.Application xl;
        private void ThisAddIn_Startup(object sender, System.EventArgs e)
        {
            ...
            wb.Close(false);
            exWb.Close(false);
            for (int i = 90; i <= 195; i++)//These are the rows in "workbooks to create.xlsm" I will traverse
                // ws.UsedRange.Rows.Count //Edit this for which workbooks (names and descriptions) you wish to create
            {
                ...
                ", fileNumber);
                        ...
                        dest = wbs.Range["A296:B300"];
                        source.Copy();
                        wbs.Activate();
                        wbs.Range["A296"].Select();
                        wbs.Paste();


                        int topOfLegend = 291;
                        source = wbsSource.Range["A28:E28"];
                        dest = wbs.Range["A292:E292"];
                        source.Copy();
                        wbs.Activate();
                        wbs.Range["A292"].Select();
                        wbs.Paste();

                        wbs.Rows[292].RowHeight = wbsSource.Rows[28].RowHeight;
                        (wbs.Rows[292] as Excel.Range).WrapText = false;
                        for (int a = 2; a <= 291; a++)
                        {
                        ...
                        wbs.Range["A296:B296"].Font.Bold = false;
                        //DRAWINGS
                ...
                ", fileNumber);
                        ...
                        dest = res.Range["A277:B280"];
                        source.Copy();
                        res.Activate();
                        res.Range["A277"].Select();
                        res.Paste();

                        res.Range["A277:B277"].Merge();

                        topOfLegend = 272;
                        source = reSource.Range["A29:V29"];
                        dest = res.Range["A273:V273"];
                        source.Copy();
                        res.Activate();
                        res.Range["A273"].Select();
                        res.Paste();
                        (res.Rows[273] as Excel.Range).WrapText = false;

                        for (int j = 22; j <= 19 + maxFurthest; j++)
                        {
                            source = res.Range["T73"];
                            dest = res.Cells[273, j];
                            source.Copy();
                            res.Cells[273, j].Select();
                            res.Paste();
                        }

                        for (int rn = 277; rn < 323; rn++)
                        {
                            res.Rows[rn].RowHeight = exWb.Sheets["Resources"].Rows[rn + (118 - 277)].RowHeight;
                        }
                        res.Range["F1"].Select();
                        for (int a = 3; a <= 272; a++)
                        {
                        ...
                        res.Rows[277].RowHeight = 21;
                        res.Range["A277:B277"].Font.Bold = false;
                        res.Range["B278:B281"].Borders[Excel.XlBordersIndex.xlEdgeRight].Weight =
                            Microsoft.Office.Interop.Excel.XlBorderWeight.xlMedium;
                        res.Range["I273"].Borders[Excel.XlBordersIndex.xlEdgeRight].LineStyle =
